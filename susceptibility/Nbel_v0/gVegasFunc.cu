...
__device__
double func(float* rx, float wgt)
{
    double result;
    cuFloatComplex  vkqt;
    cuFloatComplex  vkqb;
    ...
    for(int m=0; m<N; m++) {
        ...
        for(int n=m; n<N; n++) {
            ...
            if(m==n){
                ...
                Grkq[IDX2F(m,n,N)] = cuCdivf(
                    cuCmulf(thekq[m], phikq[n + 2]), 
                    thekq[N]
                );
                //Gakq[IDX2F(m,n,N)] = complex(Grkq[IDX2F(m,n,N)].real, -Grkq[IDX2F(m,n,N)].imag);
                Gakq[IDX2F(m,n,N)] = cuConjf(Grkq[IDX2F(m,n,N)]);

                //Grk[IDX2F(m,n,N)] = thek[m] * phik[n + 2] / thek[N];
                Grk[IDX2F(m,n,N)] = cuCdivf(
                    cuCmulf(thek[m], phik[n + 2]), 
                    thek[N]
                );
                //Gak[IDX2F(m,n,N)] = complex(Grk[IDX2F(m,n,N)].real, -Grk[IDX2F(m,n,N)].imag);
                Gak[IDX2F(m,n,N)] = cuConjf(Grk[IDX2F(m,n,N)]);
            }
            //elif m < n:
            else if(m<n) {

                Grkq[IDX2F(m,n,N)] = cuCmulf(
                ...
                //Grkq[IDX2F(n,m,N)] = mp[n - m] * pvkqb[n - m] * thekq[m] * phikq[n + 2] / thekq[N];
                Grkq[IDX2F(n,m,N)] = cuCmulf(
                ...
                //Gakq[IDX2F(m,n,N)] = complex(Grkq[IDX2F(n,m,N)].real, -Grkq[IDX2F(n,m,N)].imag);
                //Gakq[IDX2F(n,m,N)] = complex(Grkq[IDX2F(m,n,N)].real, -Grkq[IDX2F(m,n,N)].imag);
                Gakq[IDX2F(m,n,N)] = cuConjf(Grkq[IDX2F(n,m,N)]);
                Gakq[IDX2F(n,m,N)] = cuConjf(Grkq[IDX2F(m,n,N)]);

                //Grk[IDX2F(m,n,N)] = mp[n - m] * pvkt[n - m] * thek[m] * phik[n + 2] / thek[N];
                Grk[IDX2F(m,n,N)] = cuCmulf(
                ...
                //Grk[IDX2F(n,m,N)] = mp[n - m] * pvkb[n - m] * thek[m] * phik[n + 2] / thek[N];
                Grk[IDX2F(n,m,N)] = cuCmulf(
                ...
                //Gak[IDX2F(m,n,N)] = complex(Grk[IDX2F(n,m,N)].real, -Grk[IDX2F(n,m,N)].imag);
                //Gak[IDX2F(n,m,N)] = complex(Grk[IDX2F(m,n,N)].real, -Grk[IDX2F(m,n,N)].imag);
                Gak[IDX2F(m,n,N)] = cuConjf(Grk[IDX2F(n,m,N)]);
                Gak[IDX2F(n,m,N)] = cuConjf(Grk[IDX2F(m,n,N)]);
            }
        }
    }
    ...
    for(int l=0; l<N; l++) {
        ...
        if((mu - rx[0] + (N2 - l) * hOmg) >= 0) {
            ...
            for(int m=0; m<N; m++) {
                //for n in range(0, N):
                for(int n=0; n<N; n++) {
                    //chi1f += Grkq[IDX2F(m,n,N)] * Grk[IDX2F(n,l,N)] * Gak[IDX2F(l,m,N)] + Grkq[IDX2F(n,l,N)] * Gakq[IDX2F(l,m,N)] * Gak[IDX2F(m,n,N)];
                    chi1f = cuCaddf(
                        chi1f,
                        cuCaddf(
                            cuCmulf(Grkq[IDX2F(m,n,N)], cuCmulf(Grk[IDX2F(n,l,N)], Gak[IDX2F(l,m,N)])),
                            cuCmulf(Grkq[IDX2F(n,l,N)], cuCmulf(Gakq[IDX2F(l,m,N)], Gak[IDX2F(m,n,N)]))
                        )
                    );
                }
            }
