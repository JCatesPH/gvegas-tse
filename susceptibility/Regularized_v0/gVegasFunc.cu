...
float func(double *rx, double wgt)
{
    ...
    cuDoubleComplex  vkt;
    cuDoubleComplex  vkb;
    // float N2 = (N - 1) / 2; // Added to constant list.

    vkqt = make_cuDoubleComplex(-V1 * sin(rx[2] * a + qy * a), V1 * sin(rx[1] * a + qx * a));
    vkqb = cuConj(vkqt);

    vkt = make_cuDoubleComplex(-V1 * sin(rx[2] * a), V1 * sin(rx[1] * a));
    vkb = cuConj(vkt);
    ...
    cuDoubleComplex thokq   = make_cuDoubleComplex(rx[0] - t0 + V0 * (cos(rx[1] * a + qx * a) + cos(rx[2] * a + qy * a)) - N2 * hOmg, Gamm);

    cuDoubleComplex thzk    = make_cuDoubleComplex(1.f, 0.f);
    cuDoubleComplex thok    = make_cuDoubleComplex(rx[0] - t0 + V0 * (cos(rx[1] * a) + cos(rx[2] * a)) - N2 * hOmg, Gamm);

    cuDoubleComplex phinpkq = make_cuDoubleComplex(1.f, 0.f);
    cuDoubleComplex phinkq  = make_cuDoubleComplex(rx[0] - t0 + V0 * (cos(rx[1] * a + qx * a) + cos(rx[2] * a + qy * a)) - (N2 - (N - 1)) * hOmg, Gamm);

    cuDoubleComplex phinpk  = make_cuDoubleComplex(1.f, 0.f);
    cuDoubleComplex phink   = make_cuDoubleComplex(rx[0] - t0 + V0 * (cos(rx[1] * a) + cos(rx[2] * a)) - (N2 - (N - 1)) * hOmg, Gamm);
    ...
    for(int ss=2; ss < N+1; ss++) {
        ...
        theskq = make_cuDoubleComplex(rx[0] - t0 + V0 * (cos(rx[1] * a + qx * a) + cos(rx[2] * a + qy * a)) - (N2 - (ss - 1)) * hOmg, Gamm); 
        theskq = cuCsub(
            cuCmul(theskq, thokq), 
        ...
        phiskq = make_cuDoubleComplex(rx[0] - t0 + V0 * (cos(rx[1] * a + qx * a) + cos(rx[2] * a + qy * a)) - (N2 - (s - 1)) * hOmg, Gamm);
        phiskq = cuCsub(
        ...
        thesk = make_cuDoubleComplex(rx[0] - t0 + V0 * (cos(rx[1] * a) + cos(rx[2] * a)) - (N2 - (ss - 1)) * hOmg, Gamm);
        thesk = cuCsub(
            cuCmul(thesk, thok), 
        ...
        phisk = make_cuDoubleComplex(rx[0] - t0 + V0 * (cos(rx[1] * a) + cos(rx[2] * a)) - (N2 - (s - 1)) * hOmg, Gamm);
        phisk = cuCsub(
            cuCmul(phisk, phink), 
        ...
        //if (mu - x[0] + ((N - 1) / 2 - l) * hOmg) < 0:
            ...
            for(int m=0; m<N; m++) {
                ...
                for(int n=0; n<N; n++) {
                    ...
                    theta = make_cuDoubleComplex(fermi(rx[0] + ((N - 1) / 2 - l) * hOmg, mu), 0);
                    chi1f = cuCadd(
                        chi1f,
                        cuCadd(
                            cuCmul(Grkq[IDX2C(m,n,N)], cuCmul(theta, cuCmul(Grk[IDX2C(n,l,N)], Gak[IDX2C(l,m,N)]))),
                            cuCmul(Grkq[IDX2C(n,l,N)], cuCmul(theta, cuCmul(Gakq[IDX2C(l,m,N)], Gak[IDX2C(m,n,N)])))
                        )
                    );
                }
            }
